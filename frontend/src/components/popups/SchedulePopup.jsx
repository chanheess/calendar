...
const SchedulePopup = ({ isOpen, mode, eventDetails, onClose, selectedCalendarList }) => {
  ...
  const fetchScheduleNotifications = async (eventId) => {
    try {
      const response = await axios.get(`/api/schedules/${eventId}/notifications`, {
  ...
  const fetchRepeatDetails = async (repeatId) => {
    ...
    try {
      const response = await axios.get(`/api/repeats/${repeatId}`, {
        withCredentials: true,
        headers: { "Content-Type": "application/json" },
      });
  ...
  const handleSave = async () => {
    try {
      if (mode === "create") {
        await axios.post("/api/schedules", getScheduleData(), {
            withCredentials: true,
            headers: { "Content-Type": "application/json" },
        });
        ...
        if (scheduleData.repeatId) {
          ...
          await axios.patch(`/api/schedules/${scheduleData.id}?repeat=${isRepeatEnabled}`, getScheduleData(), {
              withCredentials: true,
              headers: { "Content-Type": "application/json" },
          });
    ...
    try {
      if (mode === "edit") {
        if (scheduleData.repeatId) {
          ...
          await axios.delete(`/api/schedules/${scheduleData.id}/calendars/${scheduleData.calendarId}`, {
              withCredentials: true,
          });
